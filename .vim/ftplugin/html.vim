...
" In normal mode, F6 inserts a latex template.
noremap <buffer> <F6> :if strpart(getline(1),0,6) !~ "^<\[Hh]\[Tt]\[Mm]\[Ll]>"<CR>0read ~/.Vim/html-template.vim<CR>normal /<\/title\\|<\/TITLE<CR><CR>:endif<CR>3jf>a

" In insert mode, F6 looks back to find the last uncompleted <> tag, and
...
" between.
inoremap <buffer> <F6> <Esc>:call <SID>OneLineCompletion()<CR>i
